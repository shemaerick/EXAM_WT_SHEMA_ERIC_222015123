...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO playlist(PlaylistID, UserID, PlaylistName) VALUES (?, ?, ?)");
    $stmt->bind_param("iss", $PlaylistID, $UserID, $PlaylistName);
    // Set parameters and execute
    $PlaylistID = $_POST['book_id'];
    $UserID = $_POST['ride_id'];
    $PlaylistName = $_POST['passenger_id'];
...
<body>
    <center><h2>Playlist Table</h2></center>
    <table border="3">
        <tr>
...
// Prepare SQL query to retrieve all playlist
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        $PlaylistID = $row['PlaylistID']; // Fetch the PlaylistID
        echo "<tr>

            <td>" . $row['PlaylistID'] . "</td>
            <td>" . $row['UserID'] . "</td>
            <td>" . $row['PlaylistName'] . "</td>
            <td><a style='padding:4px' href='delete_playlist.php?PlaylistID=$PlaylistID'>Delete</a></td> 
            <td><a style='padding:4px' href='update_playlist.php?PlaylistID=$PlaylistID'>Update</a></td> 
        </tr>";
    }
