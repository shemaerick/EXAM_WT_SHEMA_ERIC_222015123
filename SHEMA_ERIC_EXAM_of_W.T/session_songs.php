...
<head>
  ...
  <title>session_songs Page</title>
  <style>
    /* Normal link */
    a {
      padding: 10px;
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO session_songs(SessionID, SongID) VALUES (?, ?)");
    $stmt->bind_param("is", $SessionID, $SongID);
    // Set parameters and execute
    $SessionID = $_POST['book_id'];
    $SongID = $_POST['ride_id'];
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>session_songs Details</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>session_songs Table</h2></center>
    <table border="3">
        <tr>
...
// Prepare SQL query to retrieve all session_songs
$sql = "SELECT * FROM session_songs";
$result = $connection->query($sql);
// Check if there are any session_songs
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<tr>
            ...
            <td><a style='padding:4px' href='delete_session_songs.php?SessionID=$SessionID'>Delete</a></td> 
            <td><a style='padding:4px' href='update_session_songs.php?SessionID=$SessionID'>Update</a></td> 
        </tr>";
    }
